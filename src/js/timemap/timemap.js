...
TimeMap.prototype = {
	 ...
	 */
	scrollToDate: function(d, lazyLayout, animated, duration) {
		var a = false;
		...
		if (d) {
			...
			if (animated) {
				...
				var provider = util.TimelineVersion() == '1.2' ? Timeline : SimileAjax,
					...
					}, topband.getCenterVisibleDate().getTime(), time, duration);
				a.run();
			}
			else {
	...
	/** CICEROW'S MODIFICATION
	 * Scroll the timeline from a given date to another given date.
	 * During a specific duration in ms.
	 *
	 * @param {Integer} duration			Duration of the animation.
	 * @param {String|Date} [startDate]	 startDate of the animation.
	 * @param {String|Date} [endDate]	   endDate of the animation.
	 */
	animate: function(duration, startDate, endDate) {
		var startDate = startDate || 'first';
		var endDate = endDate || 'last';
		console.log(startDate,endDate,duration);
		this.scrollToDate(startDate);
		return this.scrollToDate(endDate, false, true, duration);
	},

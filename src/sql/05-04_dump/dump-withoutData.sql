...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` VIEW `carteevenement` AS select `utilisateur`.`login` AS `login`,`carte`.`titre` AS `titre`, COUNT(evenement.id)
